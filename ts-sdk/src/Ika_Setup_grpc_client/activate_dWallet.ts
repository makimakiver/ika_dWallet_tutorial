...
const DWALLET_RESULT_FILE =
  process.env.DWALLET_RESULT_FILE || "output/dwallet_result.json";
...
try {
  ...
  throw new Error(
    `Failed to load dWallet data from ${DWALLET_RESULT_FILE}: ${error}`,
  );
}
...
const userPublicOutputFromFile = objectToUint8Array(
  dwalletData.dkgRequestInput.userPublicOutput,
);
...
if (!dWalletObjectID) {
  throw new Error(
    "dWalletObjectID not found in JSON file or environment variable",
  );
}

console.log(`[Config] dWalletObjectID: ${dWalletObjectID}`);
console.log(
  `[Config] Root seed key loaded (first 10 bytes): ${Buffer.from(ROOT_SEED_KEY.slice(0, 10)).toString("hex")}`,
);
console.log(
  `[Config] User public output loaded, length: ${userPublicOutputFromFile.length}`,
);
...
async function main() {
  ...
  if (currentState !== "AwaitingKeyHolderSignature") {
    console.log(
      `[Step 3] dWallet is in ${currentState} state. Waiting for AwaitingKeyHolderSignature...`,
    );
    try {
      const dWallet = await ikaClient.getDWalletInParticularState(
        dWalletObjectID as string,
      ...
      throw new Error(
        `Failed to reach AwaitingKeyHolderSignature state: ${error.message}`,
      );
    }
  }
  ...
  if (!dynamicFields.data || dynamicFields.data.length === 0) {
    throw new Error(
      "No encrypted user secret key shares found in the table. The network may not have completed DKG yet.",
    );
  }
  console.log(
    `[Step 5] Found ${dynamicFields.data.length} encrypted share(s) in table`,
  );

  const encryptedUserSecretKeyShareId = (dynamicFields.data[0] as any).objectId;
  console.log(
    `[Step 6] encryptedUserSecretKeyShareId: ${encryptedUserSecretKeyShareId}`,
  );
  ...
  try {
    ...
    console.log(
      `[Step 11] Active dWallet public_output length: ${activeDWallet.state.Active?.public_output?.length || "N/A"}`,
    );
  } catch (error: any) {
    console.log(
      `[Step 11] Warning: Timeout waiting for Active state. The dWallet may still be processing.`,
    );
    console.log(
      `[Step 11] You can check the state later and proceed with create_Presign.ts once Active.`,
    );
  }
  ...
  console.log(
    "[SUCCESS] You can now run create_Presign.ts to request a presign.",
  );
}
