...
async function retryUntil<T>(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      console.log(
        `[Polling] Attempt ${attempt + 1}/${maxRetries} - condition not met, waiting ${intervalMs}ms...`,
      );
    } catch (error: any) {
      console.log(
        `[Polling] Attempt ${attempt + 1}/${maxRetries} - error: ${error.message}, waiting ${intervalMs}ms...`,
      );
    }
...
async function main() {
  ...
  const rawUserIkaCoins = rawUserCoins.data.filter(
    (coin: any) => coin.coinType === testnetIkaCoinType,
  );
  ...
  console.log(
    `[Step 7] Session ID: ${Buffer.from(sessionId).toString("hex").substring(0, 16)}...`,
  );
  ...
  console.log(
    `[Step 9] dWalletNetworkEncryptionKeyId: ${dWalletEncryptionKey.id}`,
  );
  ...
  console.log(
    `[Step 12] dWallet fetched. State: ${dWallet.state?.$kind || "unknown"}`,
  );

  console.log(
    "[Step 13] Using user public output from dWallet creation file...",
  );
  ...
  console.log(
    `[Step 13] User public output loaded, length: ${dkgRequestInput.userPublicOutput.length}`,
  );
  console.log(
    "[Step 13] dkgRequestInput.userPublicOutput:",
  ...
  console.log(
    `[Step 13] encryptedUserSecretKeyShareId: ${encryptedUserSecretKeyShareId}`,
  );
  ...
  const presignEvent = waitForTransactionResult.events?.find((event: any) =>
  ...
  fs.writeFileSync(
    presignFilepath,
    JSON.stringify(presignResult, null, 2),
    "utf-8",
  );
  ...
  console.log(
    "[Step 22] You can now use this presign for signing in a separate transaction.",
  );
}
