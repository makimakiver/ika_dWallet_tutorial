...
function collectCommission(ikaConfig, systemObjectRef, validatorCommissionCap, amount, tx) {
  return tx.moveCall({
    ...
    arguments: [
      ...
      amount !== null ? tx.pure(bcs.option(bcs.u64()).serialize(amount)) : tx.pure(bcs.option(bcs.u64()).serialize(null))
    ]
  });
}
...
function setApprovedUpgradeByCap(ikaConfig, systemObjectRef, packageId, digest, protocolCap, tx) {
  tx.moveCall({
    ...
    arguments: [
      ...
      digest !== null ? tx.pure(bcs.option(bcs.vector(bcs.u8())).serialize(digest)) : tx.pure(bcs.option(bcs.vector(bcs.u8())).serialize(null)),
      tx.object(protocolCap)
    ]
  });
}
